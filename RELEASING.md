# Releasing bp-calendar

## Overview

This repository releases from the `main` branch using `scripts/release.sh`.

The script handles the version bump, release commit, git tag, push, and npm publish for `@braudypedrosa/bp-calendar`.
...
git tag --list
npm view @braudypedrosa/bp-calendar version
```
...
- publish succeeds but verification lags
  - fix: wait briefly and rerun `npm view @braudypedrosa/bp-calendar version`
